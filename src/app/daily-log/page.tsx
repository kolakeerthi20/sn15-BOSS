...
export default function DailyLogPage() {
  const { dailyLogs, currentUser } = useAppStore();
  const today = new Date().toISOString().split('T')[0];

  const todayLogs = dailyLogs.filter(l => l.date === today);
  const myLogs = dailyLogs.filter(l => l.userId === currentUser?.id);
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  Team Activity Today ({todayLogs.length})
                </CardTitle>
              </CardHeader>
                    ...
                    <div key={log.id} className="border-b border-slate-100 px-5 py-3 last:border-0 dark:border-slate-800">
                      <div className="flex items-center gap-2 mb-1">
                        <Avatar className="h-6 w-6">
                          <AvatarFallback name={log.user.name} className="text-[9px]">{getInitials(log.user.name)}</AvatarFallback>
                        </Avatar>
              ...
              <CardContent className="p-0">
                {myLogs.slice(0, 7).map(log => (
                  <div key={log.id} className="border-b border-slate-100 px-5 py-3 last:border-0 dark:border-slate-800">
                    ...
                    <p className="mt-0.5 text-[10px] text-slate-400">{log.hoursWorked}h · {log.deliverablesCompleted.length} deliverables</p>
                  </div>
                ))}
                {myLogs.length === 0 && (
